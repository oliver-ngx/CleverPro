...
Two things shape this list. First, **the server is ahead of the client**: four
...
Everything else is just work. These need your opinion, and all three are
...
### 2. Where does Deploy live? *(carried over — still your call)*
...
### 3. Can people reply on a version?
...
2. **Run the tests on push.** There are 147 of them — 105 for the server, 42 for
...
A decision that was on this list is settled too. **Where somebody reads that
their role changed** is answered: pressing a teammate's face and name in the
header opens their settings page, and the notice is a row on it — "Your role
changed" on your own, "Role last changed" on somebody else's, and no row at all
for a viewer the server would refuse. Role changes, ownership transfer and
removal all moved onto that page, which is where the product spec always said
they belonged: authority moves with the person.

---

## The main work: finish what is already built

**Effort:** medium each · **Needs your decision:** decisions 1 and 3 above
...
5. **A way to leave the project.** The endpoint is there and refuses the Owner,
...
6. **Branch from an older version.** The Add Branch sheet always starts from
...
7. **Replies on a version** (decision 3), if you want them.

**Not yet:** three single-object reads — one commit, one branch, and one
branch's version list. They exist for a screen opened directly on one thing, and
the app has one URL and no routing, so there is nothing to open. Worth building
when deep links are, and not before. The fourth, one member, now has a screen:
it is what the settings page reads.

**An open question, if branches ever need deleting.** There is no way to delete
one, at either end. A domain method existed and was removed: it dropped the
branch and left everything hanging off it — the commit and push indexes, the
working copies, every ledger row naming it — so anything wired to it would have
been resolving commits on a branch that no longer existed. The real question is
what should happen to that history, and the product has not been asked it. Decide
that before writing the method again.
...
8. **Real logins.** Every request says who it is and the server believes it, so
...
9. **Stop losing everything on restart.** The whole project is a dictionary in
...
10. **Two people at the same time.** Today a write refreshes *your* tab and
    nobody else's; a second browser sees nothing until it reloads. Presence —
    the green dot — is hardcoded off for the same reason. For a product about
    working with other people this matters, but it needs 8 and 9 underneath it
    first.
11. **Somewhere to run it.** In development Vite proxies `/api` to the API so the
