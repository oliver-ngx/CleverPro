...
export function AppWindow({ children }: AppWindowProps) {
  return (
    <div className="flex min-h-dvh justify-center bg-cp-desktop font-ui sm:items-center sm:p-[24px] xl:p-[40px]">
      <div className="relative flex h-dvh w-full max-w-[1400px] overflow-hidden bg-cp-window sm:h-[min(805px,100dvh_-_48px)] sm:rounded-cp-window sm:shadow-cp-window xl:h-[min(805px,100dvh_-_80px)]">
        {children}
      </div>
    </div>
  )
