...
class Project:
    ...
    def create_branch(self, actor: str, name: str, members: Optional[set[str]] = None,
        ...
        if not name:
            name = f"branch-{next(_id_counter)}"   # auto-fill default, naming is optional
    ...
    def commit(self, actor: str, branch: str, attachment: Attachment, comment: str,
        ...
        changed_paths = list(attachment.tree_snapshot.keys()) if attachment.folder_ref else attachment.loose_files
        proposed_files = dict(attachment.tree_snapshot) if attachment.folder_ref else attachment.file_contents
    ...
    def undo(self, actor: str, target_version_label: Optional[str] = None) -> DeployRecord:
        """
        Rollback of the live domain to a previously deployed version.
        Invariant 5: history is preserved — only what's currently live changes.
        Defaults to the deployment immediately prior to the current one.
        Restores that version's REAL file content into deployed_files —
        this is a genuine content rollback, not just a label change.
        """
        self._require_role(actor, Role.MAINTAINER, "undo (rollback)")
        if len(self.deploy_history) < 2 and target_version_label is None:
        ...
        elif target_version_label not in {d.version_label for d in self.deploy_history}:
            raise ValueError(f"'{target_version_label}' was never deployed — nothing to roll back to.")
        ...
        self.deployed_files = dict(matching[-1].files) if matching else {}
    ...
    def archive_surface(self) -> list[dict]:
        ...
        version ever pushed to Main, newest first. The currently live one
        shows "Applied"; every other row shows "Undo" (clicking it rolls
        production back to that version, via undo()).
        """
        main = self.branches["main"]
        current = self.deployed_version
        return [
    ...
    def file_tree_at_version(self, branch_name: str, version_label: str) -> list[dict]:
        ...
        if not matching:
            raise KeyError(f"'{version_label}' is not a version that exists on {branch_name}.")
        return build_file_tree(matching[-1].files)
